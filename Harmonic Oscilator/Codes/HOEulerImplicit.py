...
for t in range(N-1):
    q[t+1]=(q[t]+h*p[t])/(1+h**2)
    p[t+1]=(p[t]-h*q[t])/(1+h**2)
    energy[t+1]=q[t+1]**2/2+p[t+1]**2/2

txt = "Energy plot of Euler Implicit Algorithm"
plt.figure(1)
plt.plot(q,p)
...
#plt.ylim([-125,125])
plt.grid()
plt.savefig("Plots/EulerI_{}.png".format(h),dpi=600,bbox_inches='tight')
plt.show 
plt.figure(2)
...
plt.savefig("Plots/energyEulerI_{}.png".format(h),dpi=600,bbox_inches='tight')
plt.show()
