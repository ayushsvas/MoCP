...
# -*- coding: utf-8 -*-
"""
Created on Sat Jan  4 21:54:09 2020
...
for t in range(T):
    ...
    if rho>np.random.random():
        ...
        t_x[0,-1]=t_x[0,0]
        t_x[0,0]=t_x[0,-1]
    KE[t]=1/(2*dt)-np.mean(np.diff(t_x[0,:])**2)/(2*dt**2)#calculation of kinetic energy
    PE[t]=np.mean(t_x[0,:]**2)/2
#   
#    if t>100:
#        if t==101:
#            t_x_prev=t_x
...
tau=np.linspace(0,np.size(corr),np.size(corr))/n
print(time.time()-tim)
plt.figure(1)
